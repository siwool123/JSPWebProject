...
<c:choose> 
	...
	<c:otherwise> <!-- 출력할 게시물이 있을때 -->
     ...
     <tr>
       <td align="center">${ map.totalCount - ((map.pageNum-1)*map.pageSize + loop.index) }</td>
       <td align="left"><a href="sub01_view.do?tname=${ tname }&idx=${ row.idx }">${ JSFunction.titleCut(row.title, 29) }</a></td>
       <td align="center">${ row.id }</td>
       <td align="center">${ row.visitcnt }</td>
       <td align="center">${ row.postdate }</td>
       <td align="center">
		<c:if test="${ not empty row.ofile }">
			<a href="../community/download.do?tname=${param.tname }&ofile=${ row.ofile }&sfile=${ row.sfile }&idx=${ row.idx }"><i class="fa-solid fa-paperclip"></i></a>
		</c:if>
		</td>
     </tr>
		</c:forEach>
...
<table width="100%">
	<tr>
		<td width="20%">총 ${ map.totalCount } 개   [ ${ map.pageNum } / ${ map.totalPage } 페이지 ]</td>
		<td width="*" class="text-center">${ map.pagingImg }</td>
		<td width="15%" align="right">
		<button type="button" onclick="location.href='sub01_write.do?tname=${ tname }';" >글쓰기</button>
		</td>
	</tr>
</table>
			</div>
