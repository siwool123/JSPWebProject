...
public class ReviewDAO extends JDBConnect {
	...
	public List<CommentDTO> selectList(int gidx) {
		List<CommentDTO> bbs = new Vector<CommentDTO>(); //벡터는 리스트의 일종. ArrayList와 유사
		String sql = "SELECT * FROM sua_review WHERE gidx="+gidx;
		try {
			...
			System.out.println("리뷰 조회 중 예외발생"); 
			e.printStackTrace();
		}
		return bbs;
	}
	...
	public int updateLikecnt(int idx) {
		int result = 0;
		String sql = "UPDATE sua_review SET likecnt=likecnt+1 WHERE idx=?";
		try {
			psmt = con.prepareStatement(sql);
			psmt.setInt(1, idx);
			result = psmt.executeUpdate();
		}catch(Exception e) {
			System.out.println("리뷰의 좋아요 수 증가 중 예외발생");
			e.printStackTrace();
		}
		return result;
	}
	
	//리뷰 입력위한 메소드. 폼값이 저장된 dto객체를 인수로 받는다.
	public int insertWrite(CommentDTO cdto) {
		int result=0;
		try {
			String sql = "INSERT INTO sua_review VALUES (seq_comment.NEXTVAL, ?, ?, SYSDATE, ?, 0, ?, ?, ?)";
			psmt = con.prepareStatement(sql);
			...
			System.out.println("리뷰 입력 중 예외발생 : "+result+", "+cdto.getBoard_idx()+", "+cdto.getId()+", "+cdto.getContent());
			e.printStackTrace();
		} 
		return result;
	}
	...
	public CommentDTO selectView(int idx) {
		CommentDTO cdto = new CommentDTO();
		String sql = "SELECT * FROM sua_review WHERE idx=?";
		try {
			psmt = con.prepareStatement(sql);
			psmt.setInt(1, idx);
 ...
 * 각 컬럼값추출시 1부터시작한느 인덱스와 컬럼명 둘다사용가능. 날짜인 경우getDate()메소드로 추출가능 */
			...
			System.out.println("리뷰불러오기 중 예외발생");
			e.printStackTrace();
		}
		return cdto;
	}
	...
	// 특정일련번호에 해당하는 리뷰 수정
	public int updateEdit(CommentDTO rdto) {
		int result=0;
		try {
			String sql = "UPDATE sua_review SET content=?, star=?, ofile=?, sfile=? WHERE idx=?";
			psmt = con.prepareStatement(sql);
			psmt.setString(1, rdto.getContent());
			psmt.setInt(2, rdto.getStar());
			psmt.setString(3, rdto.getOfile());
			psmt.setString(4, rdto.getSfile());
			psmt.setInt(5, rdto.getIdx());
			result = psmt.executeUpdate();
		}catch(Exception e) {
			System.out.println("리뷰 수정 중 예외발생");
			e.printStackTrace();
		}
		return result;
	}
	...
	public int deletePost(CommentDTO cdto) {
		int result=0;
		try {
			String sql = "DELETE FROM sua_review WHERE idx=?";
			psmt = con.prepareStatement(sql);
			psmt.setInt(1, cdto.getIdx());
			result = psmt.executeUpdate();
		}catch(Exception e) {
			System.out.println("리뷰 삭제 중 예외발생");
			e.printStackTrace();
		}
		return result;
	}
